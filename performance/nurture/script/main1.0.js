...
$(() => {
  ...
  if (enableHeaderMenu) {
    ...
    function resetHeaderMenuStates() {
      $(".header__top__menu__list__item__link").removeClass("active subColor2");
  ...
  if (enableSection02Slide) {
    function checkWidthAndAddDiv(sectionNumber) {
      ...
      if ($(window).width() <= 960) {
        if (!$(sectionId + " .section02__arrows").length) {
          $(sectionId).append('<div class="section02__arrows flex"></div>');
          initializeSlick(sectionNumber);
        }
      } else {
        $(sectionId + " .section02__arrows").remove();
        if (
    ...
    function initializeSlick(sectionNumber) {
      ...
      $(sectionId + " .section02__inner__wrap__list")
        .on("init", function (event, slick) {
          $(sectionId + " .section02__arrows").append(
        ...
        .slick({
          prevArrow: '<div class="section02__prev__icon flex">←</div>',
          nextArrow: '<div class="section02__next__icon flex">→</div>',
          appendArrows: $(sectionId + " .section02__arrows"),
        })
    ...
    $("[id^='section04_']").each(function () {
      ...
      $(sectionId + " .section04__wrap__list__item").each((index) => {
        ...
        $(
          ...
          $(
            sectionId + " .section04__wrap__list__item:nth-of-type(" + ind + ")"
          ).toggleClass("subColor2");
        });
      });
    });
  }
  ...
  if (enableSection07Slide) {
    ...
    function initializeSlick2(sectionId) {
      $(sectionId + " .section07__wrap__list")
        ...
        .slick({
          slidesToShow: 3,
          prevArrow: '<div class="section07__prev__icon flex">←</div>',
          nextArrow: '<div class="section07__next__icon flex">→</div>',
  ...
  // 各要素に対してcomMore__linkクラスを削除
  elements.forEach((element) => {
    element.classList.remove("comMore__link");
  });
