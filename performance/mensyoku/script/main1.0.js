...
$(() => {
  ...
  if (enableSection02Slide) {
    ...
    function initializeSlick(sectionId) {
      $(sectionId + " .section02__wrap__list")
        .on("init", function (event, slick) {
          var $numContainer = $('<div class="slick-num"></div>');
          for (var i = 0; i < slick.slideCount; i++) {
            if (i === 0) {
              $numContainer.append(
                '<div class="num active">' + (i + 1) + "</div>"
              );
            } else {
              $numContainer.append('<div class="num">' + (i + 1) + "</div>");
            }
          }
          $numContainer.appendTo($(this).siblings(".section02__wrap__arrows"));
          $numContainer.find(".num").click(function () {
            var index = $(this).index();
            $(sectionId + " .section02__wrap__list").slick("slickGoTo", index);
        ...
        .on("beforeChange", function (event, slick, currentSlide, nextSlide) {
          $(this).find(".slick-num .num.active").removeClass("active");
          $(this)
            .siblings(".section02__wrap__arrows")
            .find(".slick-num .num:eq(" + nextSlide + ")")
            .addClass("active");
        });
    }
    $("[id^='section02_']").each(function () {
      var sectionId = "#" + this.id;
      checkWidthAndAddDiv(sectionId);
    });
    ...
    function initializeSlick2(sectionId) {
      $(sectionId + " .section07__wrap__list")
        .on("init", function (event, slick) {
          ...
          $numContainer.appendTo($(this).siblings(".section07__wrap__arrows"));
          $numContainer.find(".num").click(function () {
            var index = $(this).index();
            $(sectionId + " .section07__wrap__list").slick("slickGoTo", index);
            $numContainer.find(".num.active").removeClass("active");
            $(this).addClass("active");
          });
        })
        .slick({
        ...
        .on("beforeChange", function (event, slick, currentSlide, nextSlide) {
          $(this)
            .siblings(".section07__wrap__arrows")
            .find(".slick-num .num.active")
            .removeClass("active");
          $(this)
            .siblings(".section07__wrap__arrows")
            .find(".slick-num .num:eq(" + nextSlide + ")")
